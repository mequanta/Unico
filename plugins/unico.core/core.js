define(["EventEmitter2"], function(Emitter) {
    main.consumes = [];
    main.provides = ["core", "Plugin"];
    return main;
    ...
    function main(options, imports, register) {
        ...
        function Plugin(developer, deps) {
            ...
            event.on("newListener", function(type, listener) {
                ...
                if (data === -1)
                    ...
                    if (event.listeners(type).indexOf(listener) > -1)
                        console.trace("Used 'on' instead of 'once' to listen to sticky event " + name + "." + type);
                }
            });
            ...
            var baseclass;
            this.baseclass = this.freezePublicAPI.baseclass = function() { baseclass = true; };
            ...
            function getElement(name, callback) {
                ...
                if (!callback) {
                    ...
                    if (!names[name]) {
                        throw new Error("Could not find AML element by name '" + name + "'");
                    }
